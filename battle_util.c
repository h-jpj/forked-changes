...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_CHARGE:  // charge
            ...
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_KINDLE:  // kindle
            if (gDisableStructs[battler].chargeTimer && --gDisableStructs[battler].chargeTimer == 0)
                gStatuses3[battler] &= ~STATUS4_KINDLED_UP;
            gBattleStruct->turnEffectsTracker++;
            break;
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gMovesInfo[gCurrentMove].powderMove) && (gBattlerAttacker != gBattlerTarget))
            {
                if (B_POWDER_GRASS >= GEN_6
                    && (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) || GetBattlerAbility(gBattlerTarget) == ABILITY_OVERCOAT || ABILITY_GROOMEDCOAT))
                {
                    gBattlerAbility = gBattlerTarget;
                    effect = 1;
                }
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_MAJESTIC_MOTH:
            ...
            break;
        case ABILITY_TWISTED_DIMENSIONS:
...
bool32 IsMoveMakingContact(u32 move, u32 battlerAtk)
{
    ...
    else if (!gMovesInfo[move].makesContact) {
        if (move == MOVE_DRAGON_WING && gBattleStruct->shellSideArmCategory[battlerAtk][gBattlerTarget] == DAMAGE_CATEGORY_PHYSICAL)
            return TRUE;
        else
            return FALSE;
    }
    else if (!gMovesInfo[move].makesContact) {
        if (move == MOVE_FROSTBITE && gBattleStruct->shellSideArmCategory[battlerAtk][gBattlerTarget] == DAMAGE_CATEGORY_PHYSICAL)
            return TRUE;
        else
            return FALSE;
    }
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    if (gStatuses3[battlerAtk] & STATUS3_CHARGED_UP && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    if (gStatuses3[battlerAtk] & STATUS4_KINDLED_UP && moveType == TYPE_FIRE)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    ...
    case ABILITY_RECKLESS:
        ...
        break;
    case ABILITY_IRON_FIST: //test 22
    ...
    case ABILITY_IRON_HEEL:
        if (gMovesInfo[move].kickingMove)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    case ABILITY_COMBAT_SPECIALIST:
        if (gMovesInfo[move].kickingMove || gMovesInfo[move].punchingMove)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    ...
    case ABILITY_BIG_WINGS:
        if (gMovesInfo[move].bigWingMove)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    ...
    case ABILITY_DRAGONS_MAW:
        if (moveType == TYPE_DRAGON)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_CARBON_FIBER:
        if (moveType == TYPE_STEEL)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_GLACIER:
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    else if (gMovesInfo[move].effect == EFFECT_FAIR_PLAY)
    {
        if (IS_MOVE_PHYSICAL(move))
        {
            atkStat = gBattleMons[battlerDef].spAttack;
    ...
    if (GetBattlerAbility(battlerAtk) == ABILITY_ANCIENT_IDOL)
    {
        if (IS_MOVE_PHYSICAL(move))
        {
            atkStat = gBattleMons[battlerAtk].defense;
	...
	if ((GetBattlerAbility(battlerAtk) == ABILITY_MOMENTUM_CRASH) && 
   ...
   case ABILITY_FAE_HUNTER:
    ...
    case ABILITY_GIFTED_MIND:
        if (moveType == TYPE_BUG || moveType == TYPE_DARK || moveType == TYPE_GHOST)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
            if (updateFlags)
...
static inline u32 CalcDefenseStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    if (gMovesInfo[move].effect == EFFECT_SHIELD_BASH || IS_MOVE_SPECIAL(move)) // uses sp.def stat instead of defense
    {
        defStat = spDef;
        defStage = gBattleMons[battlerDef].statStages[STAT_SPDEF];
        usesDefStat = TRUE;
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, u32 abilityDef)
{
    ...
    case ABILITY_PRISM_SCALES:
        if (IS_MOVE_SPECIAL(move))
            return UQ_4_12(0.65);
        break;
    case ABILITY_BLUBBER:
        if (IS_MOVE_SPECIAL(move))
            return UQ_4_12(0.65);
        else if (IS_MOVE_PHYSICAL(move))
            return UQ_4_12(0.65);
        break;
    case ABILITY_GROOMEDCOAT:
        if (IS_MOVE_SPECIAL(move))
            return UQ_4_12(0.75);
        else if (!IsMoveMakingContact(move, battlerAtk) && moveType == TYPE_FIRE)
            return UQ_4_12(2.0);
        else if (IsMoveMakingContact(move, battlerAtk) && moveType != TYPE_FIRE)
            return UQ_4_12(0.5);
        break;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if (moveType == TYPE_DRAGON && defType == TYPE_FAIRY
        && abilityAtk == ABILITY_OVERWHELM
        && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
    ...
    if (gMovesInfo[move].effect == EFFECT_FREEZE_DRY && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_HARD_BOIL && defType == TYPE_WATER)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_SHATTER_CRY && defType == TYPE_ICE)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_EXTINGUISH && defType == TYPE_FIRE)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_CORROSION && defType == TYPE_STEEL)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_EROSIVE_WAVE && defType == TYPE_ROCK)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_WARRIORS_EYE && defType == TYPE_DRAGON)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_DECONTAMINATE && defType == TYPE_POISON)
        mod = UQ_4_12(2.0);
    if (gMovesInfo[move].effect == EFFECT_UP_ROOT && defType == TYPE_GRASS)
        mod = UQ_4_12(2.0);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
	...
	if ((GetBattlerAbility(battlerDef) == ABILITY_MAGMA_ARMOR) && moveType == TYPE_WATER)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
    ...
    if ((GetBattlerAbility(battlerDef) == ABILITY_MAJESTIC_MOTH) && moveType == TYPE_GROUND)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
            ...
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_AVOIDED_DMG;
            RecordAbilityBattle(battlerDef, ABILITY_MAJESTIC_MOTH);
        }
    }
    
    if ((GetBattlerAbility(battlerDef) == ABILITY_METALLIC) && moveType == TYPE_GROUND)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
            ...
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_AVOIDED_DMG;
            RecordAbilityBattle(battlerDef, ABILITY_METALLIC);
        }
    }

    if ((GetBattlerAbility(battlerDef) == ABILITY_DRAGON_FLY) && moveType == TYPE_GROUND)
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
            gBattleScripting.abilityPopupOverwrite = gLastUsedAbility = ABILITY_DRAGON_FLY;
            gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_AVOIDED_DMG;
            RecordAbilityBattle(battlerDef, ABILITY_DRAGON_FLY);
        }
    }
    ...
    if ((gMovesInfo[move].ignoreTypeIfSteel) && (gBattleMons[battlerDef].type1 == TYPE_STEEL || gBattleMons[battlerDef].type2 == TYPE_STEEL || gBattleMons[battlerDef].type3 == TYPE_STEEL))
    {
        modifier = UQ_4_12(1.0);
    }
...
bool32 IsBattlerAffectedByHazards(u32 battler, bool32 toxicSpikes)
{
	...
	else if (GetBattlerAbility(battler) == ABILITY_MOUNTAINEER && !toxicSpikes)
    {
        ret = FALSE;
    }
    return ret;
...
bool32 MoveHasKindleTurnAdditionalEffect(u32 move)
{
    u32 i;
    for (i = 0; i < gMovesInfo[move].numAdditionalEffects; i++)
    {
