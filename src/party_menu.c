...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
      ...
      if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_FLY) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_FLY) == ALREADY_KNOWS_MOVE)) // If Mon can learn Fly and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
      if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_SURF) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_SURF) == ALREADY_KNOWS_MOVE)) // If Mon can learn Fly and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 4 + MENU_FIELD_MOVES);
      if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_FLASH) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_FLASH) == ALREADY_KNOWS_MOVE)) // If Mon can learn Fly and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);
      if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_DIVE) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_DIVE) == ALREADY_KNOWS_MOVE)) // If Mon can learn Fly and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 6 + MENU_FIELD_MOVES);
      if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_WATERFALL) == CAN_LEARN_MOVE || CanTeachMove(&mons[slotId], MOVE_WATERFALL) == ALREADY_KNOWS_MOVE)) // If Mon can learn Fly and action list consists of < 4 moves, add FLY to action list
